...
import { DEFAULT_RPC, BHP_NETWORK, RPC_VERSION } from "../consts";
...
/**
 * RPC Client model to query a BHP node. Contains built-in methods to query using RPC calls.
 */
export class RPCClient {
   ...
   * @param version Version of BHP node. Used to check if RPC methods have been implemented. it will default to DEFAULT_RPC found in CONST
   */
  public constructor(net: string, version = RPC_VERSION) {
    if (net === BHP_NETWORK.MAIN) {
      this.net = DEFAULT_RPC.MAIN;
    } else if (net === BHP_NETWORK.TEST) {
  ...
  /**
   * Gets the version of the BHP node. This method will never be blocked by version. This method will also update the current Client's version to the one received.
   */
  ...
  /**
   * Checks if the provided address is a valid BHP address.
   */
