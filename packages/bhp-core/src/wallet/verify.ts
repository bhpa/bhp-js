...
/**
 * Verifies a BRC2. This merely verifies the format. It is unable to verify if it is has been tampered with.
 */
export function isBRC2(brc2: string): boolean {
  try {
    if (brc2.length !== 58) {
      return false;
    }
    const hexStr = ab2hexstring(base58.decode(brc2));
...
export function isAddress(address: string): boolean {
  try {
    ...
    if (getAddressFromScriptHash(scriptHash) !== address) {
      // address is not valid BHP address, could be btc, ltc etc.
      return false;
    }
    return true;
  } catch (e) {
