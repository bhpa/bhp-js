...
export class BhpDB implements Provider {
  private url: string;

  public get name() {
    return `BhpDB[${this.url}]`;
  }
  ...
  public constructor(url: string) {
    if (settings.networks[url] && settings.networks[url].extra.bhpDB) {
      this.url = settings.networks[url].extra.bhpDB;
    } else {
      this.url = url;
    }
    log.info(`Created BhpDB Provider: ${this.url}`);
  }
