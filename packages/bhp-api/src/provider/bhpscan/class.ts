import { logging, rpc, settings, u, wallet } from "../../../../bhp-core/lib";
...
export class Bhpscan implements Provider {
  private url: string;

  public get name(): string {
    return `Bhpscan[${this.url}]`;
  }
  ...
  public constructor(url: string) {
    if (settings.networks[url] && settings.networks[url].extra.bhpscan) {
      this.url = settings.networks[url].extra.bhpscan;
    } else {
      this.url = url;
    }
    log.info(`Created Bhpscan Provider: ${this.url}`);
  }
